...
export default function setup( m ) {
  ...
  m.createSyncFromDump = function( dump, _existingObj, parent, desiredName, manualParamsMode, $scope )
  {
  ...
  m.restoreFeatures = function ( dump, obj) {
    ...
    if (!obj.features_list_is_restored.has(dump.features_list)) {
      ...
      for (let fr of (dump.features_list || [])) 
      {
         ...
         fr.$scopeParent = dump.$scopeParent;
    ...
    for (var lname of Object.keys(dump.links || {})) {
      ...
      if (arr[0] == "." || arr[0] == "~") {
        ...
        let lobj = obj.createLinkTo( {param: arr[1], 
         ...
         if (dump.$scopeParent)        
             lobj.$scopes.addScopeRef( dump.$scopeParent );
  ...
  m.createChildrenByDump = function( dump, obj, manualParamsMode )
  {
    ...
    var result_p = new Promise( (resolv, reject) => {
      ...
      function restore( i, priority ) {
        ...
        if (dump.$scopeParent) // F-SCOPE
            child_dump.$scopeParent = dump.$scopeParent;
