...
//      $scopeFor = obj.$scopes.createScope("createSyncFromDump");
    ...
    // !dump.manual => ручной объект, не добавляем его в scope
    if ($scopeFor && !_existingObj && !dump.manual) { 
    ...
    keys.forEach( function(name) {
      ...
      if (v?.needLexicalParent) {
         ...
         if (Array.isArray(v)) // там список окружений - всем назначим..
         {
            ...
            // todo optimize! возможно "набор окружений" следует именно держать как такую структуру..
            // к которой можно спокойно разные scopeFor прицеплять (снаружи)
